...
插件是代码。工作区是你的工作履历、你的薪酬、你的约束条件。它们是两回事，就该待在两个不同的目录里。插件装到你的智能体存放插件的地方；`/slushpile:onboard` 在另一个你自己保密的目录里跑。
...
2. 5个并行审阅者本应彼此不可见。在同一个上下文里做不到，而一个已经看过初筛判定的专项审阅者，会朝着同意它的方向漂移。每份报告都完整写出来再开始下一份，技能里要求的就是这个。
