...
上面这张图从「发现」开始，因为指名的公司正是从那里进入的。心里没有具体公司的用户传进来的
是一段查询，第 0 阶段会在发现之前把它解析成一份公司清单——读的是流水线其余部分读的那同一份
`preferences.yaml` 约束条件，以及那同一份 `companies.md` 历史记录，这样一次市场查询就不会
把上个月的查询已经覆盖过的东西再搜一遍。第 0 阶段只产出一份清单，别的什么都不做。从发现
往后的一切在两种模式下完全相同，正是这一点让一次查询运行得出的评估，在校准表里与一次指名
公司运行得出的评估保持可比。
