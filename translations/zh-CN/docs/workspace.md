...
那个目录会装着你完整的雇佣履历、你的薪酬数字和你的各项约束。把它放进**私有**仓库，或者干脆不放进任何仓库。引导流程会把这话对你说一遍，而且它不会替你初始化仓库，也不会加远程地址——那是一个要刻意做出的决定，不是从某个安装步骤里继承来的。
...
它会长。当一次评审说某一节内容单薄时，`/slushpile:explore-experience` 会访谈你，并把它问出来的东西写回这里，这样下一次申请就从这里起步。

### `preferences.yaml`

机器可读的那一半。薪酬计算方式和基线、地点与搬迁约束、背景审查与学历状态、你自称的差异化优势、你的文风智能体，以及 `calibration_priors`。
...
在一次评审中，某家公司此前的申请历史会被池分析师和反方读到。此前在**更高**级别上被拒是有实质影响的：招聘方看得到整份申请人跟踪记录，而后来一次更低级别的申请，读起来就是连降数级。
...
招聘启事是**逐字**存下来的，不做摘要。评审期间有3个智能体直接解析那段文字，而一次转述会悄悄抹掉它们存在就是为了核对的那些确切的资格措辞。
...
`skills/` 或 `agents/` 里没有任何东西硬编码关于你的事实。没有薪酬底线，没有地点，没有背景审查状态，没有雇主。需要其中某一项的技能在运行时从 `preferences.yaml` 读取，而且只要有个人事实漏进插件，一道 CI 门禁就会让构建失败。
