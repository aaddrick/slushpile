...
bộ nhớ tính đủ giá cho từng hồ sơ một: bạn dán một bản CV vào, nhận về một con
số, đóng tab lại, và công cụ kết thúc phiên làm việc với đúng những gì nó đã
biết lúc bắt đầu.

Bộ nhớ của Slushpile là một thư mục các tệp thuộc về bạn. Không có cơ sở dữ
...
| `preferences.yaml` | `onboard`, được `status` sửa lại | việc chấm điểm, tiêu chí loại, khâu phái đi của phần đánh giá |
...
| `applications/<company>/<role>/` | `job-board-search`, rồi tới builder | phần đánh giá, và `status` |
...
**Phát hiện của phần đánh giá → hồ sơ.** Khi một lượt đánh giá nói rằng một mục
...
**Kết quả → sổ theo dõi.** Bạn ghi lại chuyện đã xảy ra: không hồi âm, vòng sơ
loại, phỏng vấn, offer, từ chối, và ở giai đoạn nào. Không thứ gì khác trong
pipeline sinh ra được dữ liệu này, vì pipeline không bao giờ nộp bất cứ thứ gì
và không bao giờ nhìn thấy một lời hồi đáp.

**Sổ theo dõi → tiên nghiệm.** `status` hồi quy chính những dự đoán của pipeline
với các kết quả đó rồi ghi phần hiệu chỉnh vào `preferences.yaml`.

## Vì sao phần hiệu chỉnh đi vào dữ liệu của người dùng

Chỗ hiển nhiên để ghi lại “pipeline này lạc quan hơn 12 điểm về các lần nộp lạnh
vào những phòng lab tiên phong” là ở chính tác nhân đã đưa ra ước lượng đó. Đó
là chỗ sai, và sai một cách lặng lẽ: định nghĩa tác nhân được phát hành kèm
plugin, nên một sửa đổi ở đó sẽ bị bản cập nhật kế tiếp hoàn tác mà không báo
trước, để lại một pipeline *từng* được tinh chỉnh và nay thì không.
...
người phản biện lúc phái đi.

## Những quy tắc giữ cho việc hiệu chuẩn trung thực
...
hai kết quả là nhiễu khoác áo hiệu chuẩn, và một khi nó đã nằm trong bảng thì
...
nộp lạnh theo phán quyết dành cho giới thiệu nội bộ là cách một pipeline tự
thuyết phục mình rằng nó đã đúng.

**Cả hai chiều sai đều được báo cáo.** Một phán quyết INTERVIEW mà bị tự động từ
chối trong vòng 72 giờ nghĩa là phần đánh giá đã bỏ sót thứ mà một bộ lọc bắt
được trong vài giây. Một phán quyết REJECT mà lại chuyển đổi nghĩa là phần đánh
...
**Phát hiện phải nêu tên một phân khúc, một chiều, và một độ lớn.** “Pipeline bị
lệch hiệu chuẩn” là câu không hành động được và không tác nhân nào tiêu thụ được
nó. “Các phán quyết INTERVIEW cho lần nộp lạnh vào những phòng lab tiên phong đã
chuyển đổi 0 trên 7, so với mức ước lượng 12%” thì có thể được ghi thành một
tiên nghiệm và đem ra hành động.
...
gắn nhãn ước lượng là chưa hiệu chuẩn. Một tiên nghiệm tính từ hai kết quả đẩy
...
được với việc pipeline tự trôi đi.
