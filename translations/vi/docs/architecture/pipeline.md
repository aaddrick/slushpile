...
trí. Chính builder tự phái đi `explore-experience`, `adversarial-review` và
`removing-ai-tells`.

Vòng lặp ở dưới cùng là phần không có thứ tương đương trong một công cụ tối ưu
CV. Kết quả được ghi lại, `status` hồi quy những gì pipeline đã dự đoán với
...
| `agent` | Một vai được phái đi: một tác nhân con có định nghĩa riêng trong `agents/` |
...
| `writeback` | Một cạnh ghi vào bộ nhớ của không gian làm việc |

Phân biệt giữa `stage` và `agent` là chỗ đáng đọc kỹ nhất. Một hộp `agent` là
một tác nhân con có định nghĩa riêng và ngữ cảnh riêng. Trên một khung chạy
không phái đi được tác nhân con, chính những hộp đó là thứ sụp lại thành một
ngữ cảnh duy nhất, và cú sụp đó là toàn bộ khác biệt giữa một lần chạy đầy đủ
và một lần chạy đã suy giảm.
...
giờ hỏi: nó giao tài liệu cho một phần đánh giá không có lợi ích gì trong đó.
...
[the-review.md](the-review.md) nói về thứ tự phái đi, mỗi vai bị giữ lại không
